---
title: "Mental Health Awareness"
author: "James Nguyen, Fengyuan Chen, Shradha Shankar"
date: "11/13/2021"
output: html_document
---

```{r setup, include=FALSE}
source("/Users/jnguy/Documents/info201/projects/project-jam860/source/chart 1.R")
source("/Users/jnguy/Documents/info201/projects/project-jam860/source/table-summary.R")
knitr::opts_chunk$set(echo = TRUE)
```

## What is this file for?

* This file will eventually become your project report for **P02: Exploratory Data Analysis**. Specifically, you will write rmarkdown to report your exploratory data analysis.

* Please see Canvas for more details.

## ACCESSING THE /source DIRECTORY
### Example 1: Source some code and run a function
```{r}
# Example 1: Note relative path, which can be read: Up one
# directory(..), down into source (/source), and 
# then "source" an R file (data_access.R)
source("../source/data_access.R")
data_access_test()
```

### Example 2: Initialize a variable and then use it
```{r}
# Example 1: This function was "sourced" above
msg <- data_access_test(" Morgan!")
```

> `r msg` Hope you have a good day!! 

## ABOUT THIS FILE: 

This is an R Markdown document. Markdown is a simple formatting syntax for authoring HTML, PDF, and MS Word documents. For more details on using R Markdown see <http://rmarkdown.rstudio.com>.

When you click the **Knit** button a document will be generated that includes both content as well as the output of any embedded R code chunks within the document. You can embed an R code chunk like this:

```{r table, echo=FALSE}
library(knitr)
kable(table, col.names = c("Age", "Have Connections with Friends/Family", "Hours of Sleep per Day", "Meals Eaten on Average per Day"), caption = "How the connection with others have an impact on one's mental health:")
```

## Chart 1


```{r pressure, echo=FALSE}
plot(chart_1)
```

This chart was included because I wanted to know the most popular ways in which
students tended to their mental health through self-care. We believe that 
there are many ways to take care of one's mental health that they can do either
at home or by themselves. This bar graph reveals the top 7 methods that people 
do to take care of their mental health. I also chose a horizontal bar graph to
make the groups more readable. In addition, I saw students liked listening to music,
playing video games, and more.

## Chart 2

## Chart 3
